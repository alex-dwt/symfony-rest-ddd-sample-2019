...
- Docker
- Docker-compose
3.1. To run the application go to ./server and install vendor libraries
(something like "docker run --rm -it --volume $(pwd):/app prooph/composer:7.2 -vvv --ignore-platform-reqs install")
3.2. Then "docker-compose up"
3.3. Now you can work with the application through the port 80.
4. To run API tests:
